<template>
  <b-container fluid>
      <!-- <button @click="add()">click on me</button> -->
...
<script>
  export default {
    ...
    methods: {
       add(){
          this.$http.get('http://127.0.0.1:8000/api/removeclassdetails/'+this.$session.get('userid'),{
          headers: {
    ...
    .then(function(response){
     ...
     this.users=response.body.results;
      ...
      remove(item, index, button) {
       this.$http.get('http://127.0.0.1:8000/api/removeclass/'+this.$session.get('userid')+'/'+item.cid,{
          headers: {
            'Authorization': 'Bearer '+this.$session.get('jwt')
          }
